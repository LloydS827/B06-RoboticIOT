...
本仓库目前用于承载项目启动文档、调研材料、实验记录、接口规范和原型代码。阶段二已经跑通 Rerun 本地技术评测的最小闭环，阶段三已有 simulation-first Physical AI 数据包 runnable package、validator、Rerun adapter 和 CLI prototype，覆盖机器人焊接工站与机械臂抓取/分拣两个仿真样例。下一步将完成最终烟测、文档结果校准、Viewer/Blueprint 人工检查和性能冒烟，并进入阶段四自有规范与外围封装收敛。
