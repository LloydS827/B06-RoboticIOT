...
阶段四已形成 LeRobot 开放数据 import adapter、lazy loader、`import-lerobot` CLI、PushT/ALOHA/fallback profile、候选指标扩展和 Rerun 多相机引用支持。当前文档已补齐 Stage 4 运行入口、LeRobot 到 Physical AI Package 映射和样板链路记录；本地默认测试与 CLI help 已通过，真实 PushT full acceptance 和 ALOHA smoke 因本地未安装 LeRobot 可选依赖而阻塞，尚未标记通过。
