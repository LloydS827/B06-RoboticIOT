...
Stage 6 后，当前主线是 **真机数据接入与数据资产化试点**：让真实机器人作业数据进入 AI 控制器，并在 AI 控制器侧完成原始留存、清洗整理、Physical AI Package 生成、回放和训练数据准备。
...
Stage 6 的主线已经转向 AI 控制器侧真机数据接入：真实机器人作业数据通过 SDK、TCP/IP JSON、文件同步、DB 写入或后续可能的现场协议进入 AI 控制器，再完成存储、清洗、整理、Physical AI Package 生成、Rerun 回放和训练数据集准备。本阶段先明确链路、字段和边界，不新增生产 connector。
...
5. 以 Stage 6 真机接入准备资料为依据，推进 AI 控制器侧 Raw Zone、Clean Zone、Physical AI Package、回放和训练数据准备的试点边界。
6. 在真实现场接入前，明确数据脱敏、权限审计、MES/PLC/HMI 边界和 native GUI 验收的后续工作。
...
Stage 6 已新增真机数据接入与数据资产化文档包，包括 `docs/stage6/README.md`、`docs/stage6/real_robot_data_asset_module.md`、`docs/stage6/real_data_field_mapping.md` 和 `docs/real-data/README.md`。`docs/real-data/1.jpg` 与 `docs/real-data/2.jpg` 当前作为 Stage 6 真机准备截图，用于支撑系统通讯关系、字段来源、优先级和待确认问题梳理。下一步仍需在真实样本可用、脱敏和现场系统边界明确后，再推进小场景试点、权限审计、数据脱敏、MES/PLC/HMI 直连和 native GUI acceptance。
