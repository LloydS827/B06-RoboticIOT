...
Stage 7 后，当前主线是 **仿真优先小作业窗口数据试点**：当前还不具备真机接入条件，因此先用 simulated Raw/Clean fixture 模拟一个最小焊接作业窗口，为后续真实/脱敏样本进入 AI 控制器侧 Raw Zone、Clean Zone、Physical AI Package、回放和训练数据准备链路做准备。
...
- 真机数据资产化：围绕 AI 控制器建立 Raw Zone、Clean Zone、Physical AI Package、Rerun replay 和训练数据准备的主链路。
...
- **Stage 7 fixture generator**：可生成仿真小焊接作业窗口 Raw/Clean fixture，并让 Clean Zone 对齐现有 `weld_workcell` importer contract。
...
- **Stage 6 真机数据资产模块文档**：已明确真机数据进入 AI 控制器后的接入、存储、清洗、package 生成、Rerun 回放和训练数据准备边界。
...
生成 Stage 7 仿真小焊接作业窗口 Raw/Clean fixture：
...
Stage 5 的离线 handoff 仍可用：工程/机器人团队可以先导出一份离线业务目录，再由 importer contract 转成 Physical AI Package，随后统一运行 validate、summarize、candidate export、training/evaluation draft export 和 Rerun `.rrd` adapter。这条路径适合脱敏样本交换、回归测试、离线验收和现场无法直连时的临时导入。

Stage 6 已经把主线推进到 AI 控制器侧真机数据接入：真实机器人作业数据未来应通过 SDK、TCP/IP JSON、文件同步、DB 写入或后续可能的现场协议进入 AI 控制器，再完成存储、清洗、整理、Physical AI Package 生成、Rerun 回放和训练数据集准备。

Stage 7 当前承接这条路线，但不假设真机接入已经具备。工程团队优先用 Stage 7 文档确认最小作业窗口、真实/脱敏样本请求、Raw/Clean Zone 边界和后续决策口径；当前可运行路径是 simulated Raw/Clean fixture，不是生产 connector。

优先阅读：

- [Stage 7 仿真优先小作业窗口数据试点](docs/stage7/README.md)
- [Stage 7 真实/脱敏样本请求清单](docs/stage7/sample_request_checklist.md)
- [Stage 7 Raw/Clean Zone 试点约定](docs/stage7/raw_clean_zone_pilot.md)
...
5. 以 Stage 6 真机接入准备资料为依据，用 Stage 7 仿真 Raw/Clean fixture 先跑通小作业窗口的离线试点边界。
6. 在真实/脱敏样本可用后，评审字段、时间、坐标、脱敏和权限缺口，再决定 importer 演进、connector skeleton、DB/schema 或 package schema changes 是否必要。
...
目标是在真机接入条件尚未具备时，用 deterministic simulated Raw/Clean fixture 模拟一个最小焊接作业窗口，验证 Raw Zone -> Clean Zone -> `weld_workcell` importer -> Physical AI Package -> validate/summarize/candidates/training draft/Rerun 的离线闭环。本阶段为真实/脱敏样本替换做准备，不实现生产 connector、长期 DB schema 或 Physical AI Package schema changes。
...
- Stage 7 仿真优先小作业窗口数据试点文档包
- Stage 7 simulated Raw/Clean fixture generator
- real-data 真机准备截图索引
- 自研路线判断
...
- Stage 6 当前只定义真机数据接入与数据资产化试点边界，尚未新增生产 connector、TCP/IP server、SDK bridge 或 DB schema。
- Stage 7 当前只生成 simulated Raw/Clean fixture 并验证离线 importer 链路，不实现生产 connector、DB schema 或 Physical AI Package schema changes。
- Physical AI Package 仍是主数据结构；Raw Zone 和 Clean Zone 是 AI 控制器侧接入、留存和清洗整理层，不替代 package。
...
Stage 7 已在真机接入条件尚未具备的前提下，新增仿真优先小作业窗口数据试点：`scripts/generate_stage7_sim_window.py` 可生成 simulated Raw/Clean fixture，Clean Zone 对齐现有 `WeldWorkcellPackageImporter` contract，并可继续进入 Physical AI Package 的 validate、summarize、candidate export、training/evaluation draft 和 Rerun `.rrd` adapter 链路。下一步仍需用真实/脱敏焊接作业窗口替换 simulated Raw Zone，并评审字段、时间戳、坐标系、脱敏、权限和 AI 控制器存储边界后，再决定是否需要 importer 演进、connector skeleton、DB/schema 或 package schema changes。
