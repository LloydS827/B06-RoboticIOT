...
阶段四已形成 LeRobot 开放数据 import adapter、lazy loader、`import-lerobot` CLI、PushT/ALOHA/fallback profile、候选指标扩展和 Rerun 多相机引用支持。Stage 4.1 已建立独立 `uv` 环境并跑通真实 `lerobot/pusht` quick smoke 与 `lerobot/aloha_static_towel` 多相机 representative smoke；PushT full acceptance 与 Viewer/Blueprint 人工检查仍在后续记录中补齐。
