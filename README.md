...
| CLI | `physical-ai-package ...` | 工程集成、离线验收 | 安装后的标准命令行入口，是 SDK 的薄封装；Stage 11.1 增加 `doctor` 用于检查当前 SDK 环境。 |
...
1. 先运行 `physical-ai-package doctor --json`，确认当前 import path、console entrypoint 和可选依赖状态。
2. 用 Stage 8 synthetic fixture 跑通 SDK/CLI 默认链路。
3. 阅读 [B06 Python SDK](docs/sdk/README.md) 和 [SDK adoption checklist](docs/sdk/adoption_checklist.md)，确认 API、输出物和数据边界。
...
8. 当前 Stage 12A 先使用本地已到位的 H300 静态工程包做 static engineering project discovery，阅读 [Stage 12A H300 静态工程包 discovery](docs/stage12a/README.md) 和 [脱敏结构摘要](docs/stage12a/h300_static_project_structure_summary.md)，再根据 Stage 11 report 和 [Stage 8 H300 synthetic-to-real gap register](docs/stage8/h300_synthetic_to_real_gap_register.md) 人工检查字段、脱敏、权限和证据缺口。
...
当前第一样板基线仍是 **Stage 8 A01 H300 synthetic demo readiness**，但最新现场输入已进入 **Stage 12A H300 静态工程包 discovery**。H300 静态工程包已经在本地受控目录到位，可用于 SDK-first 的只读结构发现；实时 API 仍在后续开发中，本阶段不再等待首条脱敏最小作业窗口过程采样才启动学习。
...
Stage 10 adoption path 之后已形成 **Stage 11 H300 sample replacement readiness** 和 **Stage 11.1 SDK real-data onboarding hardening**。现在的下一步修正为 **Stage 12A H300 static engineering project discovery**：只读 inspect 本地 `data/H300/<local-project-run>` 静态工程包，输出可提交的脱敏结构摘要和后续 mapping 输入。Stage 12A 不实现 production connector、DB/schema、realtime API、demo UI 或 Clean Zone 自动转换，也不修改 Physical AI Package v0.1 schema。
...
- [Stage 12A H300 静态工程包 discovery](docs/stage12a/README.md)
- [Stage 12A H300 静态工程包脱敏结构摘要](docs/stage12a/h300_static_project_structure_summary.md)
...
- Stage 12A H300 静态工程包 discovery 文档和脱敏结构摘要
...
README 记录项目定位、使用入口、主链路、当前第一样板、当前能力、profile 入口、工程对接方式、快速开始和当前边界，保持在项目级概览层面。更细的执行记录、当前完成事项、下一步计划和阶段性决策记录在 [details.md](details.md) 中。SDK API 说明由 [B06 Python SDK](docs/sdk/README.md) 承载；工程团队对接字段、导出目录、验收 checklist、gap register、Stage 11 sample replacement readiness、Stage 12A static engineering project discovery 和常见错误由 Stage 5、Stage 7.1、Stage 8、Stage 11、Stage 12A 和 profile 文档共同承载。
...
Stage 9 已完成 SDK 化与用户入口收敛：B06 对外定位为 Python SDK first，`physical_ai_data` 是主产品入口，`physical-ai-package` 是标准 CLI 薄封装，`scripts/` 保留为兼容和开发期生成器。Stage 10 已完成 SDK adoption hardening：补齐 SDK API guide、adoption checklist、notebook-style walkthrough、可运行 examples、CLI JSON smoke、常见错误信息 polish 和 smoke tests。当前推荐先运行 `physical-ai-package doctor --json`，再从 Stage 8 synthetic Clean Zone 通过 `run_weld_workcell_pipeline`、`physical-ai-package run-weld-workcell` 或 `examples/` 跑通默认链路；Stage 11 和 Stage 11.1 已补齐 sample replacement readiness 与 real-data onboarding，用 `assess_h300_sample_readiness(...)`、`physical-ai-package assess-h300-readiness`、`examples/sdk_real_data_onboarding.py` 和 `PipelineResult.to_dict()` 检查候选 Clean/Raw roots 并记录输出索引。Stage 12A 已将路线修正为先 inspect 本地 H300 静态工程包，沉淀脱敏结构摘要和 Stage 12B/13 输入；实时 API、production connector、DB/schema、demo UI 和 Clean Zone 自动转换均在本阶段之外。
