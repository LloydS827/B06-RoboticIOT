...
Clean Zone 文件列表保持 `job.json`、`frames.csv`、`process.csv`、`events.csv`、`review_labels.csv` 和 images；H300 新字段先通过这些文件和 source artifact 引用落地，真实样本评审后再决定是否扩展 contract。
...
- 统一任务上下文：`job_window_id`、`work_order_id`、`task_id`、`station_id`、`robot_id`、`welder_id`、`part_id`、`seam_id`、`task_name`、`created_at`。
- 统一时间单位为秒。
- 将机器人位姿、相机位姿/标定引用、图像引用和点云引用写入 `frames.csv`。
- 将焊接电流、电压、送丝、气体、行走速度、工艺参数和质量结果写入 `process.csv`。
- 将执行事件、阶段切换、异常或报警写入 `events.csv`。
- 将模型输出、人工修正和人工复核标签写入 `review_labels.csv` 或通过 source artifact 引用。
