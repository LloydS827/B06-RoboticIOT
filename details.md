...
- 完成 Stage 11.1 SDK real-data onboarding hardening：本阶段把 Stage 9/10/11 的 SDK-first 主链路进一步产品化为 candidate real/de-identified Clean Zone 的工程 onboarding 入口，不声称仓库已有真实 H300 样本，也不声称真实数据试点完成。
- Stage 11.1 代码与示例更新：
...
1. 执行 Stage 12A H300 静态工程包 discovery：先完成 redacted-safe inspector 和 `physical-ai-package inspect-h300-static path/to/project --json`，再用本地 `data/H300/<local-project-run>` 做 manual smoke，只记录脱敏结构结论。
